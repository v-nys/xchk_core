...
def course_local_map_view(request,course_title,uid):
    ...
    substructure = [({'title' : dependent.title, 'url' : reverse(dependent.uid + "_view")},[{'title':dependency.title, 'url' : reverse(dependency.uid + "_view")} for dependency in dependencies]) for (dependent, dependencies) in structure if dependent.uid in fixpoint]
